...
class FlowMatchingClassUncond(nn.Module):
    ...
    def sample(
        ...
        """Uses flow matching model to generate novel images.
        
        Randomly samples num_samples initial noises of size given by img_wh (based on seed, if given)
        and iteratively denoises them according to flow matching denoising procedure. Returns denoised
        images.
        ...
        t = t.to(self.device)
        while t[0].item() < 1:
            x_t = x_t + step_size * self.unet(x_t, t)
            t += step_size
        return x_t
...
class FlowMatchingClassCond(nn.Module):
    ...
    def forward(self,
        ...
        """Outputs predicted deviation from true imae for random initial noises and timesteps given class.

        Randomly samples initial noises and timesteps, and produces intermediate
        noises by taking convex combinations of the initial noises and correspoinding
        sample images in x. Outputs predicted deviations from the sample image for
        each intermediate noise, given the class of the true image. 
        
        Args:
            x_1: (N, C, H, W) input tensor of sample images.
            c: int, class of img
            p_uncond: float [0, 1], probability that class of img is masked in training
    ...
    def sample(
        ...
        while t[0].item() < 1:
            ...
            u_uncond = self.unet(x_t, c, t, mask_uncond)
            u_cond = self.unet(x_t, c, t, mask_cond)
